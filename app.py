...
if uploaded_file:
    ...
    # Price calculation with detrending
...
#     x_variables = st.multiselect("Select X-variables (Features)", options=[col for col in df.columns if col != y_variable])
    ...
    # Price vs Target scatterplot (NEW)
    ...
    for feature in x_variables:
        if pd.api.types.is_numeric_dtype(df[feature]):
            # Skip transforms for Price if monotonic constraint is active
            if feature == 'Price':
    ...
    if st.button("Run AutoML"):
        ...
        # ==============================================
        # SHAP and Elasticity Analysis (FIXED)
        # ==============================================
        shap_values = {}
        model_preds = {}
        ...
        for model_info in top_models:
            ...
            try:
                ...
                # Force negative SHAP for Price if monotonic constraint exists
        ...
        # Elasticity calculation (FIXED)
        elasticity_results = {}
        for model_id, shap_df in shap_values.items():
            try:
                shap_df = shap_df.T.squeeze() if isinstance(shap_df, pd.DataFrame) else shap_df
                mean_prediction = np.abs(np.mean(model_preds[model_id]))  # Absolute mean to prevent sign flips
        ...
        # ==============================================
        # Debugging: Partial Dependence Plot (NEW)
        # ==============================================
